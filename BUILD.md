...
Developer notes for building the extension, and the source-review notes AMO
asks for. The user-facing description lives in [`README.md`](./README.md).
...
The dev server serves the same page as a plain web app, so the board, the
settings panel and every card can be worked on without reloading an extension.
...
| `npm run build`         | Type-check and build to `dist/` (Chrome / Edge)    |
...
until Firefox restarts Firefox only keeps unsigned add-ons installed for the
session, there is no permanent "load unpacked" outside Nightly/Developer
Edition with `xpinstall.signatures.required` turned off in `about:config`. For
a lasting install, submit the built `dist/` folder for signing at
...
- **`base: "./"` in `vite.config.ts`** inside an extension `/` is the
  extension root, not the page's folder, so absolute asset URLs 404.
- **`manifest.config.json`** MV3, declaring `chrome_url_overrides.newtab`
  plus `browser_specific_settings.gecko.id`, which Firefox requires to keep an
  add-on's identity (and its storage) stable across reloads. `vite.config.ts`
  emits it into `dist/` as `manifest.json`, per target.
- **The `favicon` permission** (Chrome build only), which lets `faviconUrl()`
  use Chrome's `_favicon` endpoint: icons the browser already holds, served
  offline with no third-party request. In a plain tab (and in Firefox, which
  has no such endpoint) it falls back to Google's favicon service.
...
│   └── layout/      # app chrome (header, clock)
├── features/
│   ├── background/  # wallpaper: store, presets, gradient editor, render layer
│   ├── checklist/   # the corner to-do card
│   ├── config/      # import / export of the whole configuration
│   ├── greeting/    # the rotating greeting
│   ├── news/        # feed catalogue, parser, desks, cards, host access
│   ├── settings/    # the right-hand settings panel
│   ├── sites/       # the board: store, types, bubble, board, form
│   └── weather/     # Open-Meteo client, card and settings
├── hooks/           # shared hooks (use-clock)
├── lib/
│   ├── asset-store.ts  # IndexedDB blob storage (uploads)
│   ├── color.ts        # hex ↔ rgb for the colour pickers
│   ├── url.ts          # URL normalisation, favicon derivation, safety checks
...
New shadcn/ui components drop in with `npx shadcn@latest add <component>`
`components.json` is already configured (new-york style, neutral base, CSS
variables).
...
`src/features/background/background-store.ts` follows the same shape under
`mainboard.background`, and each other feature keeps its own key.
...
- **`version` + `migrate` are wired from day one.** Bump the version and add a
  branch to `migrate` whenever a persisted shape changes, so boards already
  saved in someone's browser keep loading.

The store owns CRUD plus the two invariants that must not be duplicated in a
component: URL normalisation and deduplication. Tag filtering and ordering are
derived at render time in `site-board.tsx` deliberately not stored, to avoid
...
## Notes for AMO source review

This extension is built with Vite, which transpiles TypeScript, bundles modules
and minifies output, so Mozilla requires the unprocessed source alongside the
signed build.

**Build environment:** any OS (Linux, macOS, Windows) the build is pure
Node/npm with no native dependencies. Node.js v22.x (tested with v22.23.1),
...
package. No other manual steps, environment variables or secrets are involved.

`manifest.json` is generated from `manifest.config.json` by the small
`manifest()` plugin in `vite.config.ts`: the Firefox build drops the `favicon`
permission, which only exists in Chrome and which AMO's linter flags as
invalid. Nothing else differs between the two targets `npm run build`
produces the Chrome package from the same sources.
...
for. Access is used for exactly one thing (an HTTP GET of the feed URL) and the
extension has no content scripts, so it never touches page content.
...
**On the two `innerHTML` warnings the linter reports in the bundle:** both sit
inside React DOM, in its `dangerouslySetInnerHTML` property handler
(`react-dom` 19.x, present in every React build). No application code in `src/`
uses `innerHTML` or `dangerouslySetInnerHTML`. Feed and API responses are
parsed with `DOMParser`, and nothing parsed out of them is ever inserted as
markup: the parser reads `textContent` for the words, and attributes (`url`,
`src`, `href`) for the addresses (`src/features/news/feed-parser.ts`). Every
one of those addresses then passes `isSafeHttpUrl` or `safeImageUrl`
(`src/lib/url.ts`) before reaching an `href` or an `<img src>`, so only
`http(s)` survives.
